...
paths_file = os.path.join(current_dir, "paths.yml")

with open(paths_file, "r") as R:
...
spark = (
    ...
    .appName("MySQL to Parquet ETL")
    .config("spark.jars", "/opt/airflow/jars/mysql-connector-j-9.5.0.jar")
    .config("spark.driver.extraClassPath", "/opt/airflow/jars/mysql-connector-j-9.5.0.jar")
    .config("spark.executor.extraClassPath", "/opt/airflow/jars/mysql-connector-j-9.5.0.jar")
    .getOrCreate()
)
